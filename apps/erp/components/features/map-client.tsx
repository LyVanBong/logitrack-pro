...
export default function MapClient() {
  ...
  if (!mounted) {
    return (
      <div className='flex h-full w-full animate-pulse items-center justify-center rounded-lg border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-800'>
        <span className='font-medium text-slate-400'>Đang tải bản đồ...</span>
      </div>
    )
  }
  ...
  const locations = {
    ...
    truck1: [21.01, 106.0] as [number, number], // Somewhere between HN and HP
    truck2: [15.0, 108.5] as [number, number] // Between DN and HCM
  }
  ...
  return (
    <div className='relative z-0 h-full w-full overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800'>
      <MapContainer
        center={[16.0, 106.0]}
        zoom={6}
      ...
      >
        ...
        <Marker position={locations.truck1} icon={truckIcon}>
          <Popup className='rounded-lg'>
            <div className='text-sm font-semibold'>Xe 29C-123.45</div>
            <div className='mb-1 text-xs text-slate-500'>Tài xế: Lưu Văn A</div>
            <div className='flex inline-flex items-center gap-1 rounded bg-blue-50 px-2 py-1 text-xs font-medium text-blue-600'>
        ...
        <Marker position={locations.truck2} icon={truckWarningIcon}>
          <Popup>
            <div className='text-sm font-semibold'>Xe 15C-678.90</div>
            <div className='mb-1 text-xs text-slate-500'>Tài xế: Trần Kim B</div>
            <div className='flex inline-flex items-center gap-1 rounded bg-amber-50 px-2 py-1 text-xs font-medium text-amber-600'>
